...
export class DashboardNutricionistaPage implements OnInit {
  ...
  alunosFiltrados = computed(() => {
    ...
    return this.alunos().filter(
      (a) => a.alunome.toLowerCase().includes(t) || a.alumatricula.includes(t),
    );
  });
  ...
  getRefeicoesDoDia(data: string): any[] {
    ...
    return links
      .map((cr) => this.refeicoes().find((r) => r.refcodigo === cr.carref_refcodigo))
      .filter(Boolean);
  }
  ...
  async salvarCardapioDia() {
    ...
    try {
      ...
      if (!card) {
        ...
        card = await this.http
          .post<any>(`${this.api}/cardapios/`, payload, { headers })
          .toPromise();
      }

      const linksAtuais = this.cardapioRefeicoes().filter(
        (cr) => cr.carref_carcodigo === card.carcodigo,
      );
      ...
      for (const refId of refIdsNovas) {
        if (!refIdsAtuais.has(refId)) {
          await this.http
            .post(
              `${this.api}/cardapio-refeicoes/`,
              { carref_carcodigo: card.carcodigo, carref_refcodigo: refId },
              { headers },
            )
            .toPromise();
      ...
      for (const link of linksAtuais) {
        if (!refIdsNovas.has(link.carref_refcodigo)) {
          await this.http
            .delete(`${this.api}/cardapio-refeicoes/${link.carrefcodigo}/`, { headers })
            .toPromise();
          sofreuModificacao = true;
        }
      }
      ...
      if (sofreuModificacao) {
        await this.http
          .patch(`${this.api}/cardapios/${card.carcodigo}/`, { cardata: card.cardata }, { headers })
          .toPromise();
      }
  ...
  novaRefeicao = {
    refnome: '',
    refcalorias: null as number | null,
    refproteina: null as number | null,
    refcarboidrato: null as number | null,
    _imagemPreview: null as string | null,
  };
  arquivoImagem = signal<File | null>(null);

  ingredientesSelecionados = signal<Set<number>>(new Set());
  alergiasSelecionadas = signal<Set<number>>(new Set()); // Novo para refalergias

  abrirModalNovaRefeicao() {
    this.novaRefeicao = {
      refnome: '',
      refcalorias: null,
      refproteina: null,
    ...
    this.ingredientesSelecionados.set(new Set());
    this.alergiasSelecionadas.set(new Set());
    this.modalRefeicaoAberto.set(true);
  }

  fecharModal() {
    this.modalRefeicaoAberto.set(false);
  }

  onImagemSelecionada(event: any) {
    const file = event.target.files[0];
    if (file) {
      this.arquivoImagem.set(file);
      const idx = URL.createObjectURL(file);
      this.novaRefeicao._imagemPreview = idx;
    }
  }

  toggleIngrediente(id: number) {
    const set = new Set(this.ingredientesSelecionados());
    if (set.has(id)) set.delete(id);
    else set.add(id);
    this.ingredientesSelecionados.set(set);
  }

  ingredienteSelecionado(id: number) {
    return this.ingredientesSelecionados().has(id);
  }

  toggleAlergiaRefeicao(id: number) {
    const set = new Set(this.alergiasSelecionadas());
    if (set.has(id)) set.delete(id);
    else set.add(id);
    this.alergiasSelecionadas.set(set);
  }

  alergiaRefeicaoSelecionada(id: number) {
    return this.alergiasSelecionadas().has(id);
  }

  async salvarRefeicao() {
    this.salvandoRefeicao.set(true);
    try {
      const headers = new HttpHeaders({ Authorization: `Bearer ${this.auth.getToken()}` });
      const formData = new FormData();

      formData.append('refnome', this.novaRefeicao.refnome);
      formData.append('refnutcodigo', this.nutricionistaId().toString());
      if (this.novaRefeicao.refcalorias)
        formData.append('refcalorias', this.novaRefeicao.refcalorias.toString());
      if (this.novaRefeicao.refproteina)
        formData.append('refproteina', this.novaRefeicao.refproteina.toString());
      if (this.novaRefeicao.refcarboidrato)
        formData.append('refcarboidrato', this.novaRefeicao.refcarboidrato.toString());

      const file = this.arquivoImagem();
      if (file) {
        formData.append('refimagem', file);
      }

      // Arrays para muitos-para-muitos
      const ingredientesArray = Array.from(this.ingredientesSelecionados());
      const alergiasArray = Array.from(this.alergiasSelecionadas());

      // No Django DRF, ManyToMany geralmente aceita array
      // Mas se não aceitar no formData bruto, enviamos uma string delimitada ou múltiplos appends (depende de como o parser age)
      // Envia múltiplos valores com a MESMA chave para simular List
      ingredientesArray.forEach((id) => formData.append('refingredientes', id.toString()));
      alergiasArray.forEach((id) => formData.append('refalergias', id.toString()));

      const respostaRef = await this.http
        .post<any>(`${this.api}/refeicoes/`, formData, { headers })
        .toPromise();

      // Já salva The rest (se o backend já tratou ingredientes no POST, não precisa do loop abaixo,
      // mas vamos manter caso o backend esteja esperando chamadas separadas para refeicao_ingrediente)
      const refId = respostaRef.refcodigo;

      // Opcionalmente, pode ser necessário criar na tabela de juncao Refeicao_Ingrediente
      // Se o serializers.py já fizer isso automaticamente, perfeito.
      // Caso contrário:
      // for(let ingId of ingredientesArray) {
      //   await this.http.post(`${this.api}/refeicao-ingredientes/`, { refingrefcodigo: refId, refingingcodigo: ingId }, {headers}).toPromise();
      // }
      ...
      console.error(e);
      alert('Erro ao criar refeição. Detalhes no console.');
    } finally {
      this.salvandoRefeicao.set(false);
    }
  }
}
